...
def _predict_all_individuals(embeddings, individual_ids, config):
...
def cache_predictions(train_embeddings, val_embeddings, config):
    ...
    train_df = pd.read_csv(config.TRAIN_GROUND_TRUTH_CSV)
    val_df = pd.read_csv(config.VAL_GROUND_TRUTH_CSV)

    train_predictions = _predict_all_individuals(train_embeddings, individual_ids, config)
    val_predictions = _predict_all_individuals(val_embeddings, individual_ids, config)

    train_out = _save(train_df, train_predictions, config.TRAIN_PREDICTIONS_CSV)
    val_out = _save(val_df, val_predictions, config.VAL_PREDICTIONS_CSV)
    print(f"Saved -> {config.TRAIN_PREDICTIONS_CSV}")
    print(f"Saved -> {config.VAL_PREDICTIONS_CSV}")
