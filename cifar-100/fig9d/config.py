"""
All paths, hyperparameters, and pool constants for reproducing Fig. 9(d) of
the PERTINENCE paper: "Inputs dispatched to either shufflenetv2_x0_5,
mobilenetv2_x1_4, or repvgg_a2" (CIFAR-100, page 8's Fig. 9 caption, read
directly off the figure). Passed in explicitly as `config` to every
function in the shared dispatcher/, dispatcher_analysis/, and eda/
...
Sibling variant: ../fig9c/config.py (Fig. 9(c)'s three-model subset, which
swaps mobilenetv2_x1_4 for mobilenetv2_x0_75 — a real, distinct chenyaofo
checkpoint not on Fig. 4b's six-model plot, see fig9c/config.py's docstring
and ../models/model_loader.py for why). Shared across both variants: the
...
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))                    # cifar-100/fig9d/
...
# Model pool — Fig. 9(d)'s exact three-model subset, read directly off the
# figure caption, MFLOPs-ascending. All three are also on Fig. 4b's
# six-model plot, so no extra checkpoint download was needed for this
# variant.
NUM_CLASSES = 3
MODEL_NAMES = ["shufflenetv2_x0_5", "mobilenetv2_x1_4", "repvgg_a2"]
...
# gets reported afterward). Same value as fig9c/config.py — same
# extractor, same FC output size (3 classes either way), so it doesn't
# move. Within 1% of the paper's own published dispatcher-overhead figure
# for CIFAR-100/ShuffleNetV2 (Table 5: 24.17 MFLOPS).
...
# real final validation. Shared with fig9c/config.py so both variants carve
