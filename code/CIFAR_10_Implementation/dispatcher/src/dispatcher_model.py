"""
Trains one dispatcher FC head (Linear EMBEDDING_DIM -> 4) and predicts with it.
...
def train_fc(train_embeddings, train_labels, penalty_matrix, class_weights, device):
    """Train Linear(EMBEDDING_DIM, NUM_CLASSES) on the given embeddings/labels under
    the penalized loss. Returns the trained weight matrix W (4, EMBEDDING_DIM) and
    ...
    fc = nn.Linear(c.EMBEDDING_DIM, c.NUM_CLASSES).to(device)
